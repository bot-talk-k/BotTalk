  ...
  try {
    ...
    startMessagePoller(botToken, ilinkUserId, (contextToken) => {
      ...
      const welcomeMsg =
        ...
        `实际情况是，如果你连续给自己发送十条甚至七八条而一句回复都没有，也会被微信以"触发限流"而暂停，一直到你从微信端发送一次消息才会通畅（你会收到我们发给你的确认消息）；如果你不发消息超过一两天，通道也被微信关闭。\n\n` +
        `这种情况我们能帮你的：\n` +
        `• 在你最后一次互动后的第 22 小时，自动提醒你保活\n` +
        `• 你回复后秒回确认——如果你没收到确认消息，则通道已断，你需要重新扫码续连\n` +
