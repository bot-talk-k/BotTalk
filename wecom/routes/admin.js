...
router.get('/stats', (req, res) => {
  try {
    ...
    const pending = db.prepare("SELECT COUNT(*) AS c FROM channels WHERE status = 'pending_userid'").get().c;
    ...
    res.json({ success: true, data: { users, channels, channels_pending: pending, pushes, push_success: pushOk, pushes_24h: pushes24h } });
  } catch (err) {
...
router.get('/users', (req, res) => {
  try {
    const rows = db.prepare(`
      SELECT u.id, u.nickname, u.wecom_userid, u.role, u.is_disabled, u.created_at,
    ...
    res.json({ success: true, data: rows.map(u => ({
      ...u,
      wecom_userid: u.wecom_userid ? u.wecom_userid.slice(0, 12) + '…' : null,
    })) });
  } catch (err) {
...
// GET /user-activity — 用户活跃度(注册趋势 + 推送量 + 活跃分桶 + 人均通道)
